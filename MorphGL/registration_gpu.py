...
table = {
        'batching': { # prepare(...) ==> Generator that produces (batch_x, batch_y, batch_adjs) with CPU/GPU
            'cpu': prepare_salient,
            'gpu+pcie': prepare_ducati,
        },
        'transferring': { # gather_pinned_tensor_rows(cpu_pinned_tensor, gpu_idx) ==> tensor on GPU
